...
docker run --name base_1 -d --net mysql_test_net -e MYSQL_USER=user  -e 'MYSQL_PASS=test' mysql-5.5; sleep 10
docker run --name base_2 -d --net mysql_test_net mysql-5.5; sleep 10
...
docker run --name base_1 -d --net mysql_test_net -e MYSQL_USER=user  -e 'MYSQL_PASS=test' -e 'DB_NAME=db_1,test_1' mysql-5.5; sleep 10
docker run -it --rm --net mysql_test_net -e 'MYSQL_MODE=backup' -e 'DB_REMOTE_HOST=base_1' -e 'DB_REMOTE_USER=user' -e 'DB_REMOTE_PASS=test' -v ${DIR_VOLUME}/backup:/tmp/backup mysql-5.5; sleep 10
...
docker run --name base_1 -d --net mysql_test_net -e 'MYSQL_MODE=master' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' -e 'DB_NAME=db_1,test_1' mysql-5.5; sleep 10
...
echo
echo "-- Create slave"
docker run --name base_2 -d --net mysql_test_net -e 'MYSQL_MODE=slave' -e 'REPLICATION_HOST=base_1' -e 'DB_REMOTE_USER=user' -e 'DB_REMOTE_PASS=pass' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' mysql-5.5; sleep 10
...
echo "-- Backup master"
mkdir -p ${DIR_VOLUME}/backup
docker run -it --rm --net mysql_test_net -e 'MYSQL_MODE=backup' -e 'DB_REMOTE_HOST=base_1' -e 'DB_REMOTE_USER=user' -e 'DB_REMOTE_PASS=pass' -v ${DIR_VOLUME}/backup_master:/tmp/backup mysql-5.5 --master-data --single-transaction; sleep 10
echo
echo "-- Restore slave from master-file"
docker run --name base_3 -d --net mysql_test_net -e 'MYSQL_MODE=slave' -e 'REPLICATION_HOST=base_1' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' -e 'MYSQL_RESTORE=/tmp/backup/backup.last.bz2' -v ${DIR_VOLUME}/backup_master:/tmp/backup  mysql-5.5; sleep 10
docker exec -it base_1 mysql -uroot -e 'INSERT INTO test_1.foo (name) VALUES ("Tom");'; sleep 5
docker run --name base_4 -d --net mysql_test_net -e 'MYSQL_MODE=slave' -e 'REPLICATION_HOST=base_1' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' -e 'MYSQL_RESTORE=default' -v ${DIR_VOLUME}/backup_master:/tmp/backup  mysql-5.5; sleep 10
docker exec -it base_3 mysql -uroot -e 'SELECT * FROM test_1.foo;' | grep -c -w "Tom"
echo
echo "-- Backup slave"
docker run -it --rm --net mysql_test_net -e 'MYSQL_MODE=backup' -e 'DB_REMOTE_HOST=base_4' -e 'DB_REMOTE_USER=user' -e 'DB_REMOTE_PASS=pass' -v  ${DIR_VOLUME}/backup_slave:/tmp/backup mysql-5.5 --dump-slave; sleep 10
echo
echo "-- Restore slave from slave-file"
docker run --name base_5 -d --net mysql_test_net -e 'MYSQL_MODE=slave' -e 'REPLICATION_HOST=base_1' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' -e 'MYSQL_RESTORE=/tmp/backup/backup.last.bz2' -v ${DIR_VOLUME}/backup_slave:/tmp/backup  mysql-5.5; sleep 10
...
docker run --name restore_1 -d --net mysql_test_net -e 'MYSQL_MODE=master' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' -e 'MYSQL_RESTORE=default' -v ${DIR_VOLUME}/backup_master:/tmp/backup mysql-5.5; sleep 10
docker run --name restore_2 -d --net mysql_test_net -e 'MYSQL_MODE=slave' -e 'REPLICATION_HOST=restore_1' -e MYSQL_USER=user -e 'MYSQL_PASS=pass' -e 'DB_REMOTE_USER=user' -e 'DB_REMOTE_PASS=pass'  mysql-5.5; sleep 10
